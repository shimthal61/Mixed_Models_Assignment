...
We have two response times that are far above 3600ms. We can approach this new information in a few different ways. We could choose to ignore the outliers, as they can be informative about the subject-area. Alternatively, we could remove the outliers from the dataset entirely. Since there is no theoretically *correct* way to progress, I believe that the best approach is to build two models - one with and one without the outliers. If both results yield a similar narrative, then we can be confident that the effect exists regardless of the influential observations. Let's create a new dataset without the outliers.
...
Similar to question one, we seem to have problems with the normality of Residuals. We can use the  `descdist()` function to have a look at how our data are distributed. 
...
Unsurprisingly, our reaction time data seems to be better represented by the Gamma distribution. Our next step is to build a factorial mixed model specify that that our data are of Gamma distribution. Like earlier, we can build the maximal feasible model using the `buildmer()` function.
