...
Please follow the link below for the full repository for this assignment:

https://github.com/shimthal61/Mixed_Models_Assignment
...
Let's first create our maximal model using the `lmer()` function. We're asking for `Response_Time` to be predicted by the fixed effect, `Context`, plus the random effect of `Item`, plus the random effect of `Subject`. This means that we're modelling an individual baseline response time for each of our items and random effects. Within our random effects, our model takes into consideration that the difference between the three levels of our experimental factor might vary between subjects and items. Finally, we're passing this all on to a variable called `q1_model_max`.
...
In this situation, we can build a model and specify that that our data are of Gamma distribution. We can build this using the `buildmer()` function, ensuring that we specify that `family = Gamma`. Gamma mixed models are often much harder to fit, so the random effect structure might have to be much simpler.

```{r}
q1_gamma_buildmer <- buildmer(Response_Time ~ Context +
                            (1 + Context | Subject) +
                            (1 + Context | Item),
                          data = q1_data_tidied,
                          family = Gamma,
                          buildmerControl = buildmerControl(direction = 'order',
                                                            quiet = TRUE))
formula(q1_gamma_buildmer)
```

This is problematic - the model `buildmer()` has chosen as the maximal feasible formula is too simplistic:

`Response_Time ~ 1 + Context`.

The maximal model that is actually capable of converging excludes our random effects. To increase the parameters in our model, we'll need to modify the integer scalar within the `nAGQ` argument. Setting `nAGQ = 0` is less accurate that having it at its default `nAGQ = 1`, but it results in a higher chance of successful convergence [(Source: Stack Exchange)](https://stats.stackexchange.com/questions/544937/when-is-it-appropriate-to-set-nagq-0-in-glmer). 

Let's build a new model with `nAGQ = 0`.

```{r}
q1_gamma_model_nAGQ0 <- buildmer(Response_Time ~ Context +
                           (1 + Context | Subject) +
                           (1 + Context | Vignette),
                        family = Gamma,
                        nAGQ = 0,
                        ...
                        buildmerControl = buildmerControl(direction = 'order',
                                                          quiet = TRUE))
formula(q1_gamma_model_nAGQ0)
```

This MFM is much more inclusive of our parameters. Later on, when interpreting our results, we'll examine both models to see whether the same story emerges for both models.
...
We are told that we have two influential observations. We can `arrange()` the `Response_Time` so that the largest Response times appear at the top of our tibble.
...
We can see here that we have two response times - 5240ms and 4613ms that are well above the others. We can approach this new information in a few different ways. We could choose to include the outliers, as they can be informative about the subject-area. Alternatively, we could remove the outliers from the dataset entirely. Since there is no theoretically *correct* way to progress, I believe that the best approach is to build two models - one with and one without the outliers. If both results yield a similar narrative, then we can be confident that the effect exists regardless of the influential observations. Let's create a new dataset without the outliers.

```{r}
q1_data_removed <- q1_data_tidied %>% 
  filter(Response_Time < 4600)
...
Now that we have all three of our models built, we can interpret the results using the `summary()` function

**Model with normality assumptions**

```{r}
summary(q1_MFM_model)
```

**Model with Gamma assumptions**

```{r}
summary(q1_gamma_model_nAGQ0)
```
...
All three of our models have given us similar outputs. More of the variability in our random effects can be attributed to our subjects as oppose the the items. We are particularly interested in our fixed effects. Due to how we previously set up our contrasts, the `Intercept` corresponds to our `Neutral` contexts level. Crucially, in all of our models, `Negative` contexts differed significantly from `Neutral` contexts. All of the models also told us that `Positive` contexts did not differ significantly from `Neutral` contexts. Since all of the models have given us similar outputs, I have decided to continue using the unmodified model, as I feel this is more in line with ethical statistical reporting. 
...
Following this, we run to run pairwise comparisons where we control for the family-wise error rate. We're going to use the `emmenas()` function from the `{emmeans}` package to run our multiple comparisons:
...
We built a mixed model to investigate the influence of Item Context (Neutral vs. Negative vs. Positive) on Reaction Time. After finding that our model violated normality assumptions, we built 2 new models. One of them modeled our data using the Gamma distribution, the other modeled our data which excluded the outliers. All three of our models revealed the same significant effects. As such, we decided to continue with our original model.

Post hoc comparisons applying Tukey correction confirmed that words appearing in Negative contexts resulted in significantly faster response times than words appearing in Positive contexts (t(568)=3.67, *p* < .001), but not Neutral contexts (t(568)=2.11, *p* = .089). The difference in response times given for words in Neutral contexts and Positive Contexts was not statistically significant (*p* = .264).

The results were carried out using the buildmer package (Voeten, 2022), which uses the lme4 package (Bates, Maechler, Bolker, Walker, 2015), to fit the linear mixed models for the response time measure in R version 4.1.1 (R Development Core Team). Normality assumptions were assessed using the performance package (Lüdecke et al., 2021). Deviation coding was used for each of the two experimental factors (Barr et al., 2013). Pairwise comparisons conducted with the emmeans package (Lenth, 2021) were used to investigate the interaction for the response time measure. Below, we report regression coefficient estimates, 95% confidence intervals, and *p* values for the intercept and fixed effects. Degrees of freedom were approximated using the Kenward-Roger method. Restricted maximum likelihood estimation was used for the reporting of parameters. For pairwise comparisons, we report the *t*-values and *p*-values. 

```{r, echo=FALSE}
tab_model(q1_MFM_model, show.df = TRUE)
```
...
Similar to question one, we seem to have problems with the normality of Residuals and have a couple of outliers. We can use the  `descdist()` function to have a look at how our data are distributed. 

```{r}
descdist(q2_data_tidied$RT)
```

Our reaction time data doesn't seem to follow the normal distribution. Let's visualise the distribution using a histogram with density along the y-axis, and overlay the plot with a translucent density plot.
...
Unsurprisingly, our reaction time data seems to be better represented by the Gamma distribution. The next step is to build a factorial mixed model, making sure we specify our data as having Gamma distribution. We can build the maximal feasible model using the `buildmer()` function.

```{r}
q2_gamma_buildmer_model <- buildmer(RT ~ StoryEmotion * FaceExpression +
                           (1 + StoryEmotion * FaceExpression | Subject) +
                           (1 + StoryEmotion * FaceExpression | Vignette),
                         family = Gamma,
                         data = q2_data_tidied,
                        buildmerControl = buildmerControl(direction = 'order',
                                                          quiet = TRUE))
formula(q2_gamma_buildmer_model)
```

Like before, the model `buildmer()` has chosen as the maximal feasible formula is too simplistic:

`RT ~ 1 + FaceExpression * StoryEmotion`.

The maximal model that is actually capable of converging excludes our random slopes. Let's set `nAGQ = 0` and try again.

```{r}
q2_gamma_model_nAGQ0 <- buildmer(RT ~ StoryEmotion * FaceExpression +
                           (1 + StoryEmotion * FaceExpression | Subject) +
                           (1 + StoryEmotion * FaceExpression | Vignette),
                        family = Gamma,
                        nAGQ = 0,
                        ...
                        buildmerControl = buildmerControl(direction = 'order',
                                                          quiet = TRUE))
formula(q2_gamma_model_nAGQ0)
```

This MFM is much more inclusive of our parameters.

Let's also check to see where our influential observations lie.

```{r}
(q2_results <- check_outliers(q2_maximal_model))
plot(q2_results, type = "bars")
```

We are told that we have two influential observations. We can `arrange()` the `RT` so that the largest reaction times appear at the top of our tibble.
...
It does not appear as though we have any particularly influential observations. In this case, it is sensible to leave our dataset as it is.

## Interpreting the Interaction

Let's compare our factorial model with normality assumptions against our new gamma model.

```{r}
summary(q2_maximal_model)
```

This is as we predicted earlier when examining our interaction plot. We have no significant main effects of `StoryEmotion` or `FaceExpression`, but we do have a significant interaction between the two variables. How about in our `gamma_model_nAGQ0`?

```{r}
summary(q2_gamma_model_nAGQ0)
```

This is interesting - the Gamma model has a very different output from the other model, with our *p*-values equal to or near 1. I suspect something major has been violated in the gamma model when nAQG is set to 0, perhaps due to the dataset not being as large as it could have been. A number of small issues have most likely combined together and led to an issue that affects the model parameter estimation. In this context, is it worth referring back to the George Box Quote:

"All models are wrong, but some are useful."

Although our Gamma model takes into account the normality violations, we are left with a model which tells us very little about the data. Our initial model, although violating the normality assumptions to a degree, is able to generate a much more useful narrative for our dataset. Moreover, since our Gamma model produced an insignificant interaction, there is no need to perform pairwise comparisons on the model

Let's now generate some pairwise comparisons with this factorial model to see what's driving the effects. We need to specify our model, what interaction we want to run the t-tests on, and whether we want to change the correction method. By default, `{emmeans}` uses the Tukey correction method, which will suffice for us since only a few of our comparisons are going to be theoretically meaningful. We can, therefore, carry out a Bonferroni adjustment manually by multiplying the calculated *p*-value by the number of theoretically meaningful comparisons. 

```{r}
emmeans(q2_maximal_model, pairwise ~ StoryEmotion*FaceExpression, adjust = "none")
```

There are only two theoretically sensible comparisons to make. these are the ones where we’re comparing like with like - `FaceExpression` of the same type (Anger or Fear) preceded by one version of our `StoryEmotion` factor vs. the other:

`Anger FaceExpression` with `Anger StoryEmotion` vs `Anger FaceExpression` with `Fear StoryEmotion`

`Fear FaceExpression` with `Fear StoryEmotion` vs `Fear FaceExpression` with `Anger StoryEmotion`

Our two meaningful comparisons are the first and last ones produced in our output. To carry out our Bonferroni adjustment, all we need to do is multiply the *p*-values by the number of meaningful comparisons - 2. Since both *p*-values < .001, multiplying them by 2 still results in *p* < .001. We can see that the interaction is being driven by Angry Face Expressions preceded by Angry and Fearful Story Emotions, as well as Fearful Face Expressions preceded by Angry and Fearful Story Emotions.

## Writing Up Results

We built a 2 (Story Emotion: Anger vs. Fear) x 2 (Face Expression: Anger vs. Fear) fully factorial mixed model to investigate the influence of Story Emotion on Reaction Time to Angry or Fearful Face Expressions. After finding that our model violated normality assumptions, we built a model which modeled our data using the Gamma distribution. This model revealed no main effect of Story Emotion (t(898)<0.01, *p*=1), and no main effect of Face Expression (t(898)<0.01, *p*=1). There was also no interaction found between Story Emotion and Face Expression (t(898)=<0.01, *p*=.998). We decided to investigate our model which had violated normality assumptions, as a means to further investigate the data. Our model revealed no main effect of Story Emotion (t(898)=0.53, *p*=.599), and no main effect of Face Expression (t(898)=0.9, *p*=0.375). However, an interaction between Story Emotion and Face Expression was found (t(898)=7.51, *p*<.001).

The interaction was interpreted by conducting Bonferroni-corrected pairwise comparisons. These comparisons revealed that the interaction was driven by Angry Face Expressions being processed faster in Angry vs. Fearful Story Emotions (1,864 ms. vs. 2,612 ms., t(898) = 4.47, *p*<.001). Fearful Face Expressions were also processed faster in Angry vs. Fearful Story Emotions (2,677 ms. vs. 2,037 ms., t(898)=6.43, *p*<.001).

The results were carried out using the buildmer package (Voeten, 2022), which uses the lme4 package (Bates, Maechler, Bolker, Walker, 2015), to fit the linear mixed models for the response time measure in R version 4.1.1 (R Development Core Team). Normality assumptions were assessed using the performance package (Lüdecke et al., 2021). Deviation coding was used for each of the two experimental factors (Barr et al., 2013). Pairwise comparisons conducted with the emmeans package (Lenth, 2021) were used to investigate the interaction for the response time measure. Below, we report regression coefficient estimates, 95% confidence intervals, and *p* values for the intercept and fixed effects. Degrees of freedom were approximated using the Kenward-Roger method. Restricted maximum likelihood estimation was used for the reporting of parameters. For pairwise comparisons, we report the *t*-values and *p*-values. 

```{r, echo=FALSE}
tab_model(q2_maximal_model, show.df = TRUE)
```