...
With a *p* < .001, it is clear that we have skewed residuals. We can also check to see where our influential observations lie, using the `check_outliers()` function:
...
We are told that we have two outliers We can do a simple search for them using `filter()` to gather response times larger than 3600.
...
With this new information, we could 

We could build a new model to  


Although our model seems to break down a little a its normality of residuals, the assumptions seem mostly to have been met. ("all models are wrong, but some are useful" - George Box).
