...
Let's now compare our experimental model to a model which has dropped the fixed effect of context. First, we need to build this latter model, which we'll call our null model. This null model needs to be a subset of our full factorial model - we have to ensure that the random effects are the same in both models.
...
We can them compare our two models with the Likelihood Ratio Test within the `anova()` function:
...
In this output, we get the estimates for our three experimental conditions, as well as our pairwise comparisons, which compares our condition with each other condition in all possible combinations. 