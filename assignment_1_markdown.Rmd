...
Our model contains the fixed main effects of `StoryEmotion` *plus* the main effect `FaceExpression` *plus* the interaction between the two factors. For our random effects, we have modeled the main effect of `StoryEmotion` to differ for each of our subjects, and to differ for each of our Vignettes. `buildmer()` adds terms in order of their contribution to a certain criterion, including that the most important random slopes be included first. The default criterion assesses the significance of the change in log-likelihood. For a very helpful vignette on using `buildmer` to automatically find and compare maximal mixed models, I Recommend this Vignette by Cesko C. Voeten, 2021. Click on the image below to access it. 

<center>

[![Link to Cesko C. Voeten vignette](..)

</center>

Find more about it [here](https://cran.r-project.org/web/packages/buildmer/vignettes/buildmer.html)
...
```{r}
q2_max_feas_model <- lmer(RT ~ 1 + FaceExpression + StoryEmotion + FaceExpression:StoryEmotion + (1 + FaceExpression | Subject) + (1 + FaceExpression | Vignette),
                          data = q2_data_tidied)
```
...
```{r}
emmeans(q2_max_feas_model, pairwise ~ StoryEmotion*FaceExpression, adjust = "none")
```